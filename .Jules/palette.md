...
## 2024-05-24 - Clipped focus rings in overflow containers
**Learning:** When interactive elements like buttons are inside a parent container with `overflow: hidden` (like a segmented control), standard `outline` focus rings can get visually clipped, rendering them invisible.
**Action:** Override `outline` on these specific elements with an `inset` `box-shadow` of the same color, and set `outline: 2px solid transparent;` to maintain High Contrast Mode accessibility. For active state elements, ensure the inset shadow color contrasts with the active background color (e.g., use the dark text color `#0f172a` against the bright green active background).
