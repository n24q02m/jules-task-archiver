...
## 2024-10-24 - HTML5 Native Form Validation for Extension Popups
**Learning:** Adding validation logic like `pattern` or `maxlength` to inputs is insufficient if the primary action button is not within a semantic `<form>` element. Without a form, users cannot submit via the Enter key, and native HTML5 validation popups will not trigger when clicking the button.
**Action:** Always wrap form inputs and their corresponding action buttons in a semantic `<form>`, change the primary button to `type="submit"`, and handle the form's `submit` event instead of the button's `click` event.
