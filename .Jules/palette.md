...
## 2025-10-25 - Semantic Forms for Validation and Submission
**Learning:** Native HTML5 validation attributes (like `pattern` or `maxlength`) and implicit submission via the "Enter" key do not work if the input fields and action buttons are not wrapped in a semantic `<form>` tag.
**Action:** Always wrap input fields and their primary action button in a `<form>` element, use `type="submit"` for the primary button, and listen for the form's `submit` event (with `e.preventDefault()`) rather than the button's `click` event to enable native browser accessibility and validation features.
