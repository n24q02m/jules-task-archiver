...
## 2026-10-26 - Native Semantic Forms for Better UX
**Learning:** Native HTML5 validation attributes (`pattern`, `maxlength`) and implicit "Enter" key submission are bypassed if the primary action button and related inputs are not inside a semantic `<form>` element. Relying purely on `click` listeners prevents natural keyboard interactions.
