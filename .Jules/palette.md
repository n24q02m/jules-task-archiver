...
## 2025-06-28 - Native Form Validation and Implicit Submission
**Learning:** Native HTML5 validation attributes (`pattern`, `maxlength`) and implicit 'Enter' key submission are completely bypassed if the primary action button is not inside a semantic `<form>` element, causing users who rely on keyboard navigation to be unable to easily submit the form and resulting in bad data being submitted if validation fails.
**Action:** Always wrap form inputs and their corresponding action buttons in a semantic `<form>`, change the primary button to `type="submit"`, and handle the form's `submit` event (using `e.preventDefault()`) rather than listening directly to the button's `click` event.
