...
## 2024-10-25 - Implicit Form Submission and Native Validation
**Learning:** Native HTML5 validation attributes (`pattern`, `maxlength`) and implicit 'Enter' key submission are bypassed if the primary action button is not inside a semantic `<form>` element. Without a form, users cannot submit by pressing 'Enter', and native validation error tooltips are never shown, forcing custom validation logic.
