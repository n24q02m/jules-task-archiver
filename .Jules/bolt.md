...
**Learning:** Frequent calls to `.splice(0, n)` on large arrays to enforce a maximum length on every insert cause severe O(N^2) performance degradation due to constant element shifting.
**Action:** Implement a high-water mark buffer (e.g., waiting for `length > MAX + BUFFER` before batch-slicing elements) to batch cleanup operations and significantly reduce CPU overhead in high-frequency paths.
