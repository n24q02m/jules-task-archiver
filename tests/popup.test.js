...
describe('popup.html accessibility', () => {
  ...
  it('should use explicit visible labels for groups using legend', () => {
    ...
    assert.ok(popupHtml.includes('aria-labelledby="execModeLabel"'), 'mode group should reference legend via aria-labelledby')
    assert.ok(popupHtml.includes('aria-labelledby="scopeLabel"'), 'scope group should reference legend via aria-labelledby')
  })
})
