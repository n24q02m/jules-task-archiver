...
describe('Orchestrator Privilege Escalation Security', () => {
  ...
  it('should reject oversized CACHE_START_CONFIG payloads', () => {
    const { onMessageListeners } = setupEnvironment()
    ...
    assert.strictEqual(
      ...
      'Should reject payload larger than 50KB'
    )
  })

  it('should reject unserializable CACHE_START_CONFIG payloads', () => {
    const { onMessageListeners } = setupEnvironment()
    const listener = onMessageListeners[0]
    ...
    assert.strictEqual(
      responseData?.error,
      'Security Error: Invalid payload',
      'Should reject unserializable payloads (e.g. circular dependencies)'
    )
  })
})
