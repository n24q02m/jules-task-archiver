...
function setupEnvironment(initialStorage = {}) {
  ...
  const scriptContent =
    ...
    globalThis.test_LOG_BUFFER_LINES = LOG_BUFFER_LINES;
...
describe('state management', () => {
  ...
  it('caps the retained log at MAX_LOG_LINES, dropping the oldest lines', () => {
    const { sandbox } = setupEnvironment({})
    // MAX_LOG_LINES = 2000, LOG_BUFFER_LINES = 500
    // To trigger a trim, we need more than 2500 lines. Let's insert 2501.
...
describe('trimLog Internal', () => {
  ...
  it('should trim oldest entries if log length exceeds MAX_LOG_LINES + LOG_BUFFER_LINES', () => {
    const { sandbox } = setupEnvironment()
    const max = sandbox.test_MAX_LOG_LINES
    const buffer = sandbox.test_LOG_BUFFER_LINES
    const state = sandbox.test_state()
