...
describe('state management', () => {
  ...
  it('caps the retained log at MAX_LOG_LINES, dropping the oldest lines', () => {
    ...
    const buf = sandbox.test_LOG_BUFFER
    for (let i = 0; i < max + buf + 1; i++) sandbox.test_addLog(`line ${i}`)
    const log = sandbox.test_state().log
    assert.strictEqual(log.length, max)
    assert.strictEqual(log[log.length - 1], `line ${max + buf}`)
    assert.strictEqual(log[0], `line ${buf + 1}`)
  })
...
describe('trimLog Internal', () => {
  it('should not trim if log length is equal to MAX_LOG_LINES + LOG_BUFFER', () => {
    ...
    const buf = sandbox.test_LOG_BUFFER
    const state = sandbox.test_state()
    state.log = Array.from({ length: max + buf }, (_, i) => `line ${i}`)

    sandbox.test_trimLog()

    assert.strictEqual(state.log.length, max + buf)
    assert.strictEqual(state.log[0], 'line 0')
    assert.strictEqual(state.log[max + buf - 1], `line ${max + buf - 1}`)
  })
  ...
  it('should trim oldest entries if log length exceeds MAX_LOG_LINES + LOG_BUFFER', () => {
    ...
    const buf = sandbox.test_LOG_BUFFER
    const state = sandbox.test_state()
    // Create max + buf + 1 entries
    state.log = Array.from({ length: max + buf + 1 }, (_, i) => `line ${i}`)

    sandbox.test_trimLog()

    assert.strictEqual(state.log.length, max)
    // Should have removed the first buf + 1 entries
    assert.strictEqual(state.log[0], `line ${buf + 1}`)
    assert.strictEqual(state.log[max - 1], `line ${max + buf}`)
  })
})
