...
describe('state management', () => {
  ...
  it('caps the retained log at MAX_LOG_LINES, dropping the oldest lines', () => {
    const { sandbox } = setupEnvironment({})
    const target = sandbox.test_MAX_LOG_LINES + sandbox.test_LOG_BUFFER + 1
    for (let i = 0; i < target; i++) sandbox.test_addLog(`line ${i}`)
    const log = sandbox.test_state().log
    assert.strictEqual(log.length, 2000)
    assert.strictEqual(log[log.length - 1], `line ${target - 1}`)
    assert.strictEqual(log[0], `line ${target - 2000}`)
  })
...
describe('trimLog Internal', () => {
  it('should not trim if log length is equal to MAX_LOG_LINES + LOG_BUFFER', () => {
    ...
    const target = max + buffer
    const state = sandbox.test_state()
    state.log = Array.from({ length: target }, (_, i) => `line ${i}`)

    sandbox.test_trimLog()

    assert.strictEqual(state.log.length, target)
    assert.strictEqual(state.log[0], 'line 0')
    assert.strictEqual(state.log[target - 1], `line ${target - 1}`)
  })
  ...
  it('should trim oldest entries if log length exceeds MAX_LOG_LINES + LOG_BUFFER', () => {
    ...
    const buffer = sandbox.test_LOG_BUFFER
    const state = sandbox.test_state()
    // Create exactly target + 1 entries to hit the high-water mark
    const target = max + buffer + 1
    state.log = Array.from({ length: target }, (_, i) => `line ${i}`)

    sandbox.test_trimLog()

    assert.strictEqual(state.log.length, max)
    // Should have removed the first (target - max) entries
    assert.strictEqual(state.log[0], `line ${target - max}`)
    assert.strictEqual(state.log[max - 1], `line ${target - 1}`)
  })
})
