...
describe('state management', () => {
  ...
  it('caps the retained log at MAX_LOG_LINES, dropping the oldest lines', () => {
    ...
    for (let i = 0; i < max + buffer + 10; i++) sandbox.test_addLog(`line ${i}`)
    const log = sandbox.test_state().log
    // At exactly max + buffer + 1, it trims down to max (2000).
    // It then adds 9 more lines, ending up with 2009.
    assert.strictEqual(log.length, max + 9)
    assert.strictEqual(log[log.length - 1], `line ${max + buffer + 9}`)
