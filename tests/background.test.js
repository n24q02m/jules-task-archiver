...
describe('state management', () => {
  ...
  it('caps the retained log at MAX_LOG_LINES, dropping the oldest lines when MAX + BUFFER is exceeded', () => {
    const { sandbox } = setupEnvironment({})
    const max = sandbox.test_MAX_LOG_LINES
    const buffer = sandbox.test_LOG_BUFFER
    // Add max + buffer + 1 elements to trigger the high water mark exactly once
    // and leave the array at exactly MAX_LOG_LINES length.
    const totalLines = max + buffer + 1
    ...
    assert.strictEqual(log[0], `line ${totalLines - max}`)
  })
