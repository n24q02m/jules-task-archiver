...
// --- Tab management ---
function getTabAccountIndex(url) {
...
async function getJulesTabs() {
  ...
  return tabs
    ...
    .sort((a, b) => {
      const na = parseInt(getTabAccountIndex(a.url) || '0', 10)
      const nb = parseInt(getTabAccountIndex(b.url) || '0', 10)
      return na - nb
    })
}

function getTabLabel(tab) {
  const index = getTabAccountIndex(tab.url)
  return index ? `u/${index}` : 'default'
}
